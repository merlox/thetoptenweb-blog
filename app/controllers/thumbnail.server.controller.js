...
exports.searchThumbnails = function(req, res){
  ...
  Thumbnail.find({}).limit(req.query.limit).sort('-createdAt').skip(skipResults).exec(function(err, thumbnailsFound){
...
exports.getThumbnails = function(req, res){
  if(req.query.lastPosts){
    ...
    Thumbnail.find({}).limit(18).sort('-createdAt').skip(skipResults).exec(function(err, thumbnailsFound){
      if(err) return res.send(err);
      return res.json({
        thumbnailsFound,
        actualPage: actualPage,
